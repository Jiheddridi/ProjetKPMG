...
const CobitInteractiveCharts = {
    ...
    initializeCharts: function() {
        // Récupérer le numéro du DF courant depuis l'attribut data-df-number
        const dfNumber = document.querySelector('[data-df-number]')?.dataset.dfNumber;
        if (dfNumber) {
            // Charger les données depuis l'API pour ce DF
    ...
    /**
     * Créer le graphique en barres (Scores par Domaine)
     */
    ...
    /**
     * Mettre à jour le tableau des objectifs (affichage du risque)
     */
    updateObjectivesTable: function() {
        ...
        objectives.forEach((objective, index) => {
            ...
            // Ajout du code couleur pour le risque
            let riskClass = 'text-gray-600';
            let riskIcon = '';
            if (objective.risque === 'Risque élevé') { riskClass = 'text-red-600 font-bold'; riskIcon = '⚠️'; }
            else if (objective.risque === 'Surperformance') { riskClass = 'text-green-600 font-bold'; riskIcon = '⭐'; }
            else if (objective.risque === 'Conforme') { riskClass = 'text-blue-600 font-bold'; riskIcon = '✔️'; }
            row.innerHTML = `
                ...
                <td class="px-4 py-2 whitespace-nowrap text-sm ${riskClass}">
                    ${riskIcon} ${objective.risque}
                </td>
            `;
    ...
    /**
     * Préparer les données des objectifs (optimisé COBIT)
     */
    prepareObjectivesData: function() {
        if (this.currentData.objectives && this.currentData.objectives.length > 0) {
            return this.currentData.objectives.map((objective, index) => {
                ...
                const baseline = this.currentData.baselines[index] || 3;
                const impact = score - baseline;
                let risque = 'Conforme';
                if (score < baseline - 0.5) risque = 'Risque élevé';
                else if (score > baseline + 0.5) risque = 'Surperformance';
                return {
